-- innodb.kp_member_authority definition
SET foreign_key_checks = 0;    # 외래키 체크 설정 해제
SET foreign_key_checks = 1;    # 외래키 체크 설정

DROP TABLE kp_member_authority;

CREATE TABLE `kp_member_authority` (
                                     ...
                                     CONSTRAINT `FK2bsrmyi9ygw5ldbrs9y9gdxoe` FOREIGN KEY (`authority_name`) REFERENCES `kp_authority` (`authority_name`),
                                     CONSTRAINT `FKih8gok7f0v6ja1i2aewj3itf1` FOREIGN KEY (`member_id`) REFERENCES `kp_member` (`member_id`)
